//
//  LoadListController_V2.m
//  实现上拉和下拉刷新
//
//  Created by GangHuang on 2021/6/25.
//  Copyright © 2020 GangHuang. All rights reserved.
//  实现tableView中的下拉上拉刷新效果及相关基础概念: https://juejin.cn/post/6865636938347446279
/// 这个组件有点问题,若是请求可能会请求3次,需要注意!!! 后面若是有想法可以解决下

#import "LoadListController_V2.h"

@interface LoadListController_V2 () <UITableViewDelegate, UITableViewDataSource>
...
/** 将要下拉刷新的h状态 -1 没有下拉刷新 0 将要下拉刷新 1 松开立即刷新*/
@property (nonatomic, assign, getter=isWillHeaderRefreshing) int refreshStatusCode;
//@property (nonatomic, assign, getter=isWillHeaderRefreshing) BOOL willHeaderRefreshing;
/** 下拉刷新的h状态*/
...
@implementation LoadListController_V2
...
- (void)headerEndRefreshing{
    self.headerRefreshing = 3;
    self.refreshStatusCode = -1;
    [UIView animateWithDuration:0.25 animations:^{
        //修改tableview的内边距使得刷新时可以让header停留在用户视线内
        UIEdgeInsets inset = self.tableView.contentInset;
        inset.top -= self.header.bounds.size.height;
...
- (void)dealFooter{
    ...
    if (self.tableView.contentOffset.y >= footerOffset && (self.refreshStatusCode == -1 && self.tableView.contentOffset.y > 0)) {
