...
#import "HGAppleStorePay.h"
#import <StoreKit/StoreKit.h>

//#define goods1 @"net.ejiajx.MobileFixCar06"

@interface HGAppleStorePay()<SKPaymentTransactionObserver,SKProductsRequestDelegate>

@property (nonatomic, strong)NSString *goodsId;/**<wct20180420  商品id*/

@end

@implementation HGAppleStorePay
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(appStorePay:responseAppStorePaySuccess:error:)]) {
        [self.delegate appStorePay:self responseAppStorePaySuccess:@{@"value":transactionReceiptString} error:nil];
    }
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transaction
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(appStorePay:responseAppStorePayStatusshow:error:)]) {
        [self.delegate appStorePay:self responseAppStorePayStatusshow:@{@"value":transaction} error:nil];
    }
    ...
    for(SKPaymentTransaction *tran in transaction){
        ...
        switch (tran.transactionState) {
            ...
            case SKPaymentTransactionStateFailed:
                //                NSLog(@"交易失败");
                //失败的时候要结束订单,否则下次支付成功的时候，获取的支付凭证是上次失败的凭证，然后不仅校验会失败，订单也是重复的
                [[SKPaymentQueue defaultQueue] finishTransaction:tran];
                break;
