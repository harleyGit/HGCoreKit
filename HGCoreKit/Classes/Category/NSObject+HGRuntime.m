...
+ (NSArray *)xg_objectsWithArray:(NSArray *)array {
    ...
    // 获得属性数组
    NSArray *list = [self hg_propertiesList];
...
void *hg_ivarsKey = "hg_ivarsList";

+ (NSArray *)hg_ivarsList {
    
    // 获取关联对象
    NSArray *result = objc_getAssociatedObject(self, hg_ivarsKey);
    ...
    // 设置关联对象
    objc_setAssociatedObject(self, hg_ivarsKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return objc_getAssociatedObject(self, hg_ivarsKey);
}
